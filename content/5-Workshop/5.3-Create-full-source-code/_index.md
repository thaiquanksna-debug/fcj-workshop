---
title: "Create the full Terraform and OPA source code"
date: 2026-07-04
weight: 3
chapter: false
pre: " <b> 5.3. </b> "
...
This section creates the complete source code from zero. The reader does not need an existing repository. One bootstrap script creates all Terraform modules, root Terraform files, and OPA policies.
...
VS Code opens the empty project folder.

# 2. Create `bootstrap-source.ps1`

In VS Code:

1. Right-click the empty project folder.
2. Click **New File**.
...
4. Paste the full script below into `bootstrap-source.ps1`.
5. Save the file.

{{% notice warning %}}
The file path must be exactly `D:\mvp_private_by_default_architecture\bootstrap-source.ps1`. Do not create it inside `infra`.
{{% /notice %}}
...
Terraform needs an email address for SNS alerts. Replace the email below with the real email inbox that should receive AWS alarm emails.
...
# 5. Verify the generated source tree
...
The root module wiring created by the script is:
